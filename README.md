...
PlanDev-CLI provides a command-line interface and user-extendable Python API for interacting with an instance of PlanDev.
...
2. Install PlanDev-CLI from Github:

   ```sh
   python3 -m pip install git+https://github.com/NASA-AMMOS/plandev-cli.git@main
   ```

3. Configure access to a PlanDev host
   ...
   3. Load either your given configuration(s) or the configuration above into PlanDev-CLI:

      ```sh
      plandev-cli configurations load -i JSON_FILE
      ```
...
6. Use the `--help` flag on any command to see available subcommands and parameters. For example:
   ...
   plandev-cli --help
   ...
   plandev-cli plans --help
   ...
   plandev-cli plans download --help
   ```
...
PlanDev-CLI uses configurations to define different PlanDev hosts. Define configurations by either loading JSON configurations or manually via the CLI. Configurations persist on a per-user basis and may be shared between installations.
...
```sh
➜  plandev-cli configurations list
Configuration file location: /Users/<username>/Library/Application Support/plandev_cli/config.json
...
If you haven't been provided a JSON configuration for a host, you can create a configuration by running `plandev-cli configurations create` and follow the prompts. 
...
PlanDev-CLI maintains a persistent "session" with a PlanDev instance so multiple commands can run without needing to re-authenticate. To start a session, use the `activate` command:
...
The default role is configured by PlanDev. To change the selected role for the active PlanDev-CLI session, use the `role` command:
...
```sh
➜  plandev-cli plans download --id 42 --output sample-output.json
```
...
PlanDev-CLI configurations include a mechanism to authenticate against an external authentication service which may require additional credentials as cookies for accessing PlanDev. PlanDev-CLI will issue a post request with given JSON data to a provided authentication endpoint and persist any returned cookies in a browser-like manner for the remainder of the PlanDev-CLI session.
...
In some cases, an admin secret may be used to permit otherwise prohibited requests through Hasura (the software behind the PlanDev API). When running a command, the user may add the `--hasura-admin-secret` flag after the `plandev-cli` command to use these elevated privileges for the following command. 
...
Instead of using the CLI for interactive use cases, the underlying classes and methods behind PlanDev-CLI can be invoked directly in Python scripts.

The key constructs are:

- `plandev_cli.plandev_host.PlanDevHost`: An abstraction for a PlanDev Host, including methods for authentication and issuing requests to the PlanDev API.
- `plandev_cli.plandev_client.PlanDevClient`: A class containing common requests and reusable logic to interact with data in PlanDev.

The following example defines a `PlanDevHost` using the necessary URLs, authenticates with a command-line prompt for the user's password, and issues a simple request using one of the built-in requests.
...
# Define the PlanDev host and provide user credentials
plandev_host = PlanDevHost(GRAPHQL_URL, GATEWAY_URL)
plandev_host.authenticate(USERNAME, PASSWORD)

# PlanDevClient takes in a host and returns an object to issue requests to that host
client = PlanDevClient(plandev_host)
...
Look through the available methods in the provided `PlanDevClient` class to find ones that suit your needs.

### Adding Methods

If you need to write a custom query, you can extend the `PlanDevClient` class and add your own method. Access the PlanDev host using the `plandev_host` property. For example:

```py

# ...

class MyCustomPlanDevClient(PlanDevClient):
    def get_plan_id_by_name(self, plan_name: str) -> int:
        ...
        resp = self.plandev_host.post_to_graphql(
            my_query,
            plan_name=plan_name
        )
        return resp[0]["id"]
...
```py
# ...
client = MyCustomPlanDevClient(plandev_host)
...
If your application will be run by a user who may also be using the CLI, you may reduce the amount of code required to configure a PlanDev host and instead just use the active session. PlanDev-CLI provides a utility to retrieve a `PlanDevClient` instance from the active CLI session:

```py
from plandev_cli.utils.sessions import get_active_session_client

# client is an instance of `PlanDevClient`
...
If you have needs for authentication (e.g., a custom token system) that aren't provided by PlanDev-CLI, you can use any features supported by the [Python `requests`](https://requests.readthedocs.io/en/latest/) module's [`Session` class](https://requests.readthedocs.io/en/latest/api/#request-sessions). Instantiate a session object, manipulate/add headers/cookies/SSL certificates/etc. as necessary, and use to instantiate a `PlanDevHostSession`:
...
plandev_host = PlanDevHost(
    GRAPHQL_URL,
    GATEWAY_URL,
    session=my_custom_requests_session
)
plandev_host.authenticate(...)
client = PlanDevClient(plandev_host)
...
See this [primer](https://realpython.com/python-virtual-environments-a-primer/) or virtual environments to get set up. A quick and easy method is using Python's native `venv` package from the root of the PlanDev-CLI repo:
...
Then, install PlanDev-CLI in editable mode via Pip:

```sh
python3 -m pip install -e .
```

Now, your installation of PlanDev-CLI will update as you make changes to the source code.
...
PlanDev-CLI has unit tests and integration tests built with the [pytest](https://docs.pytest.org/) library.

#### Unit Tests

Unit tests can be run anytime and reference local test files. `test_plandev_client.py` is where unit tests are added to exercise particular methods of the `PlanDevClient` class using mocked PlanDev API responses. 
...
PlanDev-CLI generally follows the [gitflow](https://www.atlassian.com/git/tutorials/comparing-workflows/gitflow-workflow) workflow model for managing releases:
...
The full workflow for releasing a new version of PlanDev-CLI: 
